...
Usage: python eval_sam3.py --gt-folder <folder_with_gts> --pred-folder <folder_with_predictions> [--iou-type segm|bbox]
"""
...
def main():
    ...
    parser.add_argument(
        ...
        default="segm",
        choices=["segm", "bbox"],
        help="Evaluation target: segmentation masks (`segm`) or bounding boxes (`bbox`).",
    )
    ...
    for subset_name, gts in saco_gold_gts.items():
        ...
        evaluator = CGF1Evaluator(
            gt_path=gt_paths, verbose=True, iou_type=args.iou_type
        )

        pred_path = os.path.join(
            args.pred_folder,
            f"gold_{subset_name}/dumps/gold_{subset_name}/coco_predictions_{args.iou_type}.json",
        )
        summary = evaluator.evaluate(pred_path)

        metric_prefix = f"cgF1_eval_{args.iou_type}_"
        cgf1 = str(round(summary[f"{metric_prefix}cgF1"] * 100, 2))
        il_mcc = str(round(summary[f"{metric_prefix}IL_MCC"], 2))
        pmf1 = str(round(summary[f"{metric_prefix}positive_micro_F1"] * 100, 2))
